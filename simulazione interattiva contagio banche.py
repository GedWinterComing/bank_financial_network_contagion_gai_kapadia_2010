...
def generatore_matrice_adiacenza_con_z_voluto_da_binomiale(nodi_totali, z_voluto):
    ...
    # z = # totale di link / # totale di nodi = m/n perciò  # totale di link = m = zn    Nei network non diretti è invece <k>=2m/n
    ...
    # round serve perchè se z è un numero con la virgola es. 1.5 allora poi questo sarà un float anzichè un int anche se fosse ad esempio 15.0, non è comunque un int ma un float. Ciò ti da errore poi nel valutare range(.. - ..) sotto, non ho capito perchè ma anche range(round(.. - ..)) fornisce errore! 
...
def generatore_matrice_pesi_bilanci_banche(matrice_adiacenza, capital_buffer):
    ...
    bilanci_bancari_matrice_n_x_5 = np.ones((numero_banche,3), dtype=float) # per ora le colonne sono solo 3 perchè mettiamo le percentuali fissate che sono gli attivi: in ordine capital buffer, asset interbancari, mutui ovvero 0.04, 0.2 e 0.8. Poi dopo le altre due colonne che mancano saranno date CON SEGNO - da i debiti interbancari e i depositi
    ...
    for indice_colonna in range(numero_banche):
        if in_degree_delle_banche_lista_asset[indice_colonna] == 0:
            ...
            W_matrice_pesi_esposizioni_bancarie[:,indice_colonna] = W_matrice_pesi_esposizioni_bancarie[:,indice_colonna]*(asset_interbanc_totale_di_ogni_banca_A_IB_i_percentuale_bilancio / in_degree_delle_banche_lista_asset[indice_colonna])
    ...
    depositi_dei_clienti_vettore_D_passività = - (1+debiti_interbanc_vettore_L_IB) # deve avere segno meno poichè nel bilancio è nelle passività e sarebbe 1-L_IB e col meno -(1-L_IB) ma siccome hai dato segno - a L_IB diventa -(1 - (-L_IB))=-(1+L_IB)=-1-L_IB
...
# G = nx.from_numpy_array(adjacency_matrix) # ATTENZIONE che pur essendo la matrice di adiacenza quella di un grafo diretto, se non specifichi DiGraph come metodo ti disegna comunque un grafo non diretto
G = nx.from_numpy_array(matrice_adiacenza_network_bancario, create_using=nx.DiGraph()) # invece nx.from_numpy_matrix() è ora deprecated nel 2026
