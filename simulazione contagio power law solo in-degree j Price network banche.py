...
#    plt.bar(valoridelvettore, conteggi)
    fig.tight_layout()
#%%
prova=generatore_matrice_adiacenza_con_z_voluto_da_power_law_in_preferential_attachment(1000,10)
...
# results = powerlaw.Fit(jei_degree_sequence, xmin = 7) # così però non ha senso results.power_law.xmin
...
def generatore_matrice_adiacenza_con_z_voluto_da_power_law_in_preferential_attachment(nodi_totali, z_voluto):
    ...
    # SICCOME 1/10=10% DEI LINK LI HAI GIA' ASSEGNATI CON DEGREE OMOGENEO PERCIO' ORA E' RIMASTO SOLO UN 40% DA ASSEGNARE IN MANIERA OMOGENEA
    # DI CONSEGUENZA NON POSSONO ESSERE 50% E 50% LE PROBABILITA' CIOE' LE FRAZIONI DEL NETWORK. Giusto è:
    # phi_probabilita = z_voluto / (z_voluto + a_parametro_probabilita) + percentuale_link_usare_subito # di fatto viene phi = 1/2 + 1/10 = 60% = 0.6
    # ----------------
    # Questo seguendo Newman e Price e Krapivsky e Redner era per la power-law della distribuzione dell'in-degree
    # Per la distribuzione dell'out-degree prendiamo la distribuziione uniforme quindi ogni nodo è scelto veramente a caso con la stessa probabilità
    # p_j = j^{-alpha} = j^-3  per in-degree
    # p_k = 1/n  per out-degree
    # Comunque vale ugualmente:  a = z = <k> = <j>
    ...
    for ci in range(int(round(link_totali_necessari - link_per_inizializzare))):
        while True:
            if phi_probabilita > np.random.random():
                ...
                nodo_in_cui_entra_link = np.random.choice(lista_link_entranti_creati) # nodo anonimo qualunque potrebbe puntare a nodo hub, accade 1/2 delle volte
            else:
                nodo_da_cui_esce_link = np.random.randint(nodi_totali)
...
# Infine, per quella esatta, 1000 simulazioni con 1000 banche servono ben 7 ore e 36 minuti !!!
...
for z_voluto in z_average_degree_voluti_vettore:
    ...
    for simulazione in range(numero_simulazioni_per_ogni_valore_di_z):
        ...
        matrice_adiacenza_network_bancario = generatore_matrice_adiacenza_con_z_voluto_da_power_law_in_preferential_attachment(numero_banche_n_ogni_simulazione, z_voluto)
...
plt.grid()
plt.xlim(-0.2,10.8)
# plt.savefig('contagio grafico 1000 banche 1000 simulazioni ogni z da 0.5 powerlaw solo in e out omogeneo.jpg', dpi=950, transparent=False, bbox_inches="tight")
